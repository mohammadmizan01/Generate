...
const NavBar = () => {
  ...
  return (
    <nav className="flex w-full flex-col items-center">
      <div className="flex w-full items-center justify-between p-4 md:px-16 md:py-5 lg:px-24 xl:px-32">
        <Link to="/">
          <svg
            width="157"
            height="40"
        ...
        <div
          ...
          } max-md:fixed max-md:left-0 max-md:top-0 max-md:z-10 max-md:flex max-md:h-screen max-md:w-full max-md:flex-col max-md:justify-center max-md:bg-black/50 max-md:backdrop-blur flex items-center gap-8 border border-white/15 px-9 py-4 text-sm transition-colors hover:border-white/30 md:rounded-full`}
        >
          <Link to="/" onClick={() => setMobileOpen(false)} className="text-white hover:text-white/90">
            Home
          </Link>
          <Link
            to="/myprojects"
          ...
          <button
            id="close-menu"
            onClick={() => setMobileOpen(false)}
            className="aspect-square rounded-md bg-gray-900 p-2 font-medium text-white transition hover:bg-gray-800 md:hidden"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
        ...
        {!session?.user ? (
          <button
            ...
            className="hidden cursor-pointer items-center gap-3 rounded-full border border-white/15 bg-linear-to-r from-[#0000FF]/20 to-[#800080]/10 px-2 py-1 pr-4 transition hover:border-white/30 md:flex"
          >
            <div className="flex size-10 items-center justify-center rounded-full border-2 border-[#7E69B5]">
              <ArrowRight />
            </div>
            <span className="text-sm text-slate-100">Get Started</span>
          </button>
        ) : (
          <>
            <button className="rounded-full border bg-white/10 px-5 py-1.5 text-xs text-gray-200 sm:text-sm">
        ...
        <button
          ...
          className="aspect-square rounded-md bg-gray-900 p-2 font-medium text-gray-50 transition hover:bg-gray-800 md:hidden"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
